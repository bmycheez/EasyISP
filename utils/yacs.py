...
class Config(OrderedDict):
    ...
    def from_namespace(self, parsed_args, unknown_args=None):
        ...
        if unknown_args:
            nested_dict.update(
                self._separator_dict_to_nested_dict(
                    self._unknown_args_to_dict(unknown_args))
            )
        ...
        def _merge(source_cfg, other_cfg, excl, keep_existed, _cur_depth=1):
            ...
            with source_cfg.unfreeze(), other_cfg.unfreeze():
                for k, v in other_cfg.items():
                    if k not in source_cfg and excl \
                            and _cur_depth <= max_exclusive_depth:
                        raise AttributeError(
                            f'attempted to merge an attribute \
                                `{k}` that is not '
                            f'found in the source Config. \
                    ...
                    if isinstance(v, Config):
                        if isinstance(source_cfg.get(k), Config):
                            _merge(source_cfg[k], v, excl,
                                   keep_existed, _cur_depth=_cur_depth + 1)
                        else:
                            source_cfg[k] = v
                    else:
    ...
    def dump(self, save_path, ignored_keys=()):
        ...
        def _serialize(obj):
            serializable_types = \
                (bool, str, int, float, list, tuple, dict, set, type(None))
        ...
        serializable_dic = _serialize({
            k: v for k, v in self.to_dict(alphabetical=True).items()
    ...
    def string(self,
               alphabetical=False, ignored_keys=(), key_width=30, indent=0):
    ...
    def print(self, streamer=print, alphabetical=False,
              ignored_keys=None, key_width=40, indent=0):
        return streamer(self.string(
            alphabetical, ignored_keys, key_width, indent))
    ...
    def _unknown_args_to_dict(unknown_args):
        """
        Convert unknown argument list returned by `parser.parse_known_args()`
        into a dict
        :param unknown_args: list of arguments, in which the keys must starts
        with '--' and the values could be any Python literal expression
            ...
            if item.startswith('--'):
                if key and value_lst:
                    ...
                    try:
                        ...
                        raise SyntaxError(
                            'invalid argument: --{} {}'.format(key, literal))
